...
void main() {
    ...
    while (hit == 0 && i < 100)
    {
      ...
      if (push_constants.map_buffer.map[mapX][mapY] > 0) hit = 1;
      i++;
    } 
    ...
    if (drawEnd >= img_size.y) drawEnd = img_size.y - 1;

    vec4 c = vec4(0.0, 0.0, 0.0, 1.0); 

    if (gl_GlobalInvocationID.y <= drawEnd && gl_GlobalInvocationID.y >= drawStart) {
