...
layout(scalar, buffer_reference) buffer MapBuffer { int map[4][24][24]; };
...
bool isBlock(ivec3 m) {
  return m.x >= 0 && m.x < 24 && m.z >= 0 && m.z < 24 && m.y >= 0 && m.y < 4 &&
         push_constants.map_buffer.map[m.y][m.x][m.z] > 0;
}

vec4 blockColor(ivec3 m) {
  vec4 c;
  switch (push_constants.map_buffer.map[m.y][m.x][m.z]) {
  case 1:
...
void main() {
  ...
  vec3 plane_v = push_constants.plane_v * img_size.y / img_size.x;

  vec3 ray_dir = push_constants.dir + push_constants.plane_u * screen.x +
                 plane_v * screen.y;
